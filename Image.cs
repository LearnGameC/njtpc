...
public class Image
{
	...
	private static Image _createEmptyImage()
	{
		if (status != 0)
		{
			Debug.LogError((object)"CANNOT CREATE EMPTY IMAGE WHEN CREATING OTHER IMAGE");
			return null;
		}
		imgTemp = null;
		status = 2;
		...
		if (i == 500)
		{
			Debug.LogError((object)"TOO LONG FOR CREATE EMPTY IMAGE");
			status = 0;
		}
		return imgTemp;
	}
	...
	private static Image _createImage(string filename)
	{
		if (status != 0)
		{
			Debug.LogError((object)("CANNOT CREATE IMAGE " + filename + " WHEN CREATING OTHER IMAGE"));
			return null;
		}
		imgTemp = null;
		...
		if (i == 500)
		{
			Debug.LogError((object)("TOO LONG FOR CREATE IMAGE " + filename));
			status = 0;
		}
		return imgTemp;
	}
	...
	private static Image _createImage(byte[] imageData)
	{
		if (status != 0)
		{
			Debug.LogError((object)"CANNOT CREATE IMAGE(FromArray) WHEN CREATING OTHER IMAGE");
			return null;
		}
		imgTemp = null;
		datatemp = imageData;
		...
		if (i == 500)
		{
			Debug.LogError((object)"TOO LONG FOR CREATE IMAGE(FromArray)");
			status = 0;
		}
		return imgTemp;
	}
	...
	private static Image _createImage(Image src, int x, int y, int w, int h, int transform)
	{
		if (status != 0)
		{
			Debug.LogError((object)"CANNOT CREATE IMAGE(FromSrcPart) WHEN CREATING OTHER IMAGE");
			return null;
		}
		imgTemp = null;
		imgSrcTemp = src;
		...
		if (i == 500)
		{
			Debug.LogError((object)"TOO LONG FOR CREATE IMAGE(FromSrcPart)");
			status = 0;
		}
		return imgTemp;
	}
	...
	private static Image _createImage(int w, int h)
	{
		if (status != 0)
		{
			Debug.LogError((object)"CANNOT CREATE IMAGE(w,h) WHEN CREATING OTHER IMAGE");
			return null;
		}
		imgTemp = null;
		wtemp = w;
		...
		if (i == 500)
		{
			Debug.LogError((object)"TOO LONG FOR CREATE IMAGE(w,h)");
			status = 0;
		}
		return imgTemp;
	}
	...
	private static Image __createImage(byte[] imageData)
	{
		...
		{
			Debug.LogError((object)"Create Image from byte array fail");
			return null;
		}
		Image image = new Image();
		try
		...
		{
			Debug.LogError((object)("CREAT IMAGE FROM ARRAY FAIL \n" + Environment.StackTrace));
		}
		return image;
	}
