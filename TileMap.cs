...
public class TileMap
{
	...
	public static void updateMusic()
	{
		...
		if (getPosVsMainChar() && !isStopping)
		{
			...
			((Component)Sound.SoundBGLoop).GetComponent<AudioSource>().volume = volume;
			return;
		}
		AudioSource component = ((Component)Sound.SoundBGLoop).GetComponent<AudioSource>();
		component.volume -= 0.01f;
		isStopping = true;
		if (((Component)Sound.SoundBGLoop).GetComponent<AudioSource>().volume <= 0f)
		{
			((Component)Sound.SoundBGLoop).GetComponent<AudioSource>().volume = 0f;
